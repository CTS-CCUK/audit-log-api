...
then
  ...
  #Do the deployment
  kubectl set image -n cms-fr deployment/auditlogapi-fr-dev auditlogapi-fr-dev=ccicreativeuk.azurecr.io/cci-audit-log-api:$TRAVIS_BUILD_NUMBER
...
then
  ...
  #update the image
  kubectl set image -n cms-fr deployment/auditlogapi-fr-test auditlogapi-fr-test=ccicreativeuk.azurecr.io/cci-audit-log-api:$TRAVIS_BUILD_NUMBER
...
then
  ...
  #update the image
  kubectl set image -n cms-fr deployment/auditlogapi-fr-preprod auditlogapi-fr-preprod=ccicreativeuk.azurecr.io/cci-audit-log-api:$TRAVIS_BUILD_NUMBER
else
  exit 1
fi
